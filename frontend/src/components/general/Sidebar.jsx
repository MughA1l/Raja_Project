        ...
        <div className="h-screen bg-[#121927] border-r border-white flex flex-col justify-between py-4 px-3 left-0 top-0 w-60 z-50">
            ...
            <div>
                ...
                <nav className="space-y-2 mt-22">
                    <SidebarItem icon={LayoutDashboard} label="Dashboard" active={getActiveLabel()} url={'/'} />
                    <SidebarItem icon={BookOpen} label="Books" active={getActiveLabel()} url={'/Books'} />
                    <SidebarItem icon={Layers} label="Chapters" active={getActiveLabel()} url={'/Chapters'} />
                    <SidebarItem icon={Image} label="Images" active={getActiveLabel()} url={'/Images'} />
                </nav>
            </div>

            {/* Bottom Section */}
            <div className="px-2 space-y-2">
                <SidebarItem icon={Settings} label="Settings" active={getActiveLabel()} url={'/Settings'} />
                <SidebarItem icon={LogOut} label="Logout" active={getActiveLabel()} url={'/Logout'} />
            </div>
        </div>
    );
};
